...
module IdFactory
  ...
  # Ids start from FIRST_OID for compatibility with L2J. The game itself is ok
  # with ids starting from 1.
  IDS = L2IdSet.new(0..FIRST_OID &- 1)

  def load
    GameDB.transaction do |tr|
      set_all_characters_offline(tr)

      if Config.database_clean_up
        if Config.allow_wedding
          clean_invalid_weddings(tr)
        end
        clean_up_db(tr)
      end

      clean_up_timestamps(tr)
      ...
      tr.query_each(sql) { |rs| IDS << rs.read(Number::Primitive).to_i32 }

      info { "#{IDS.size &- FIRST_OID} ids loaded in #{timer} s." }
    end
  end
  ...
  private def set_all_characters_offline(tr)
    tr.exec("UPDATE characters SET online = 0")
    info "Updated character online status."
  end

  private def clean_up_db(tr)
  ...
  private def clean_invalid_weddings(tr)
  ...
  private def clean_up_timestamps(tr)
    time = Time.ms
    TIMESTAMPS_CLEAN.each { |sql| tr.exec(sql, time) }
  end
end
