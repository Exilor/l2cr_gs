class NpcAI::Rignos < AbstractNpcAI
  ...
  def on_adv_event(event, npc, player)
    ...
    when "startRace"
      ...
      if npc.script_value?(0)
        ...
        TIMER.skill.apply_effects(player, player)
        if summon = player.summon
          TIMER.skill.apply_effects(summon, summon)
        end

        if has_quest_items?(player, STAMP)
          take_items(player, STAMP, -1)
        end
      end
    when "exchange"
      player = player.not_nil!
      if get_quest_items_count(player, STAMP) >= 4
        give_items(player, KEY, 3)
        take_items(player, STAMP, -1)
      end
    when "TIME_OUT"
  ...
  def on_first_talk(npc, player)
    if npc.script_value?(0) && player.level
      htmltext = "32349.html"
    else
