class Quests::Q00016_TheComingDarkness < Quest
  # NPCs
  ...
  def initialize
    ...
    add_talk_id(HIERARCH, EVIL_ALTAR_1, EVIL_ALTAR_2, EVIL_ALTAR_3, EVIL_ALTAR_4, EVIL_ALTAR_5)
  ...
  def on_talk(npc, player)
    ...
    st2 = player.get_quest_state(Q00017_LightAndDarkness.simple_name)

    # if (st2.not_nil?) && !st2.completed?
    ...
    when State::CREATED
      htmltext = player.level >= 62 ? "31517-00.htm" : "31517-05.html"
    when State::STARTED
      npc_id = npc.id
      if npc_id == HIERARCH
        if st.cond?(6)
        ...
        htmltext = npc_id.to_s + "-00.html"
      else
        htmltext = npc_id.to_s + "-01.html"
      end
    end
