class Quests::Q00418_PathOfTheArtisan < Quest
  # NPCs
  ...
  def on_adv_event(event, npc, player)
    ...
    when "30527-06.htm"
      qs.start_quest
      give_items(player, SILVERYS_RING, 1)
      htmltext = event
    when "30527-08b.html"
      take_items(player, SILVERYS_RING, 1)
      take_items(player, BOOGLE_RATMAN_TOOTH, -1)
      take_items(player, BOOGLE_RATMAN_LEADERS_TOOTH, -1)
      give_items(player, PASS_1ST_CERTIFICATE, 1)
      qs.set_cond(3, true)
      htmltext = event
    when "30527-08c.html"
      take_items(player, SILVERYS_RING, 1)
    ...
    when "30298-02.html", "30317-02.html", "30317-03.html", "30317-05.html",
      ...
      htmltext = event
    when "30298-03.html"
      if has_quest_items?(player, KLUTOS_LETTER)
        take_items(player, KLUTOS_LETTER, 1)
        give_items(player, FOOTPRINT_OF_THIEF, 1)
        qs.set_cond(5, true)
        htmltext = event
      end
    when "30298-06.html"
      if has_quest_items?(player, FOOTPRINT_OF_THIEF, STOLEN_SECRET_BOX)
        give_items(player, PASS_2ND_CERTIFICATE, 1)
        take_items(player, FOOTPRINT_OF_THIEF, 1)
        take_items(player, STOLEN_SECRET_BOX, 1)
        give_items(player, SECRET_BOX, 1)
        qs.set_cond(7, true)
        htmltext = event
      end
    when "30317-04.html"
      give_items(player, KLUTOS_LETTER, 1)
      qs.set_cond(4, true)
      htmltext = event
    when "30317-07.html"
      give_items(player, KLUTOS_LETTER, 1)
      ...
      htmltext = event
    when "30317-10.html"
      if has_quest_items?(player, PASS_2ND_CERTIFICATE, SECRET_BOX)
        give_adena(player, 163800, true)
        give_items(player, FINAL_PASS_CERTIFICATE, 1)
        level = player.level
        if level >= 20
          add_exp_and_sp(player, 320534, 32452)
        elsif level == 19
          add_exp_and_sp(player, 456128, 30150)
        else
          add_exp_and_sp(player, 591724, 36848)
        end
    ...
    when "30317-12.html"
      if has_quest_items?(player, PASS_2ND_CERTIFICATE, SECRET_BOX)
        ...
        level = player.level
        if level >= 20
          add_exp_and_sp(player, 160267, 11726)
        elsif level == 19
          add_exp_and_sp(player, 228064, 15075)
        else
          add_exp_and_sp(player, 295862, 18424)
        end
        qs.exit_quest(false, true)
        player.send_packet(SocialAction.new(player.l2id, 3))
        qs.save_global_quest_var("1ClassQuestFinished", "1")
  ...
  def on_talk(npc, player)
    ...
    if qs.created? || qs.completed?
      ...
      when BLACKSMITH_SILVERA
        if has_quest_items?(player, SILVERYS_RING) && ((get_quest_items_count(player, BOOGLE_RATMAN_TOOTH) + get_quest_items_count(player, BOOGLE_RATMAN_LEADERS_TOOTH)) < 12)
          htmltext = "30527-07.html"
        elsif has_quest_items?(player, SILVERYS_RING) && get_quest_items_count(player, BOOGLE_RATMAN_TOOTH) >= 10 && get_quest_items_count(player, BOOGLE_RATMAN_LEADERS_TOOTH) >= 2
          htmltext = "30527-08a.html"
        elsif has_quest_items?(player, PASS_1ST_CERTIFICATE)
          htmltext = "30527-09.html"
        elsif !has_quest_items?(player, PASS_1ST_CERTIFICATE) && qs.memo_state?(10)
          htmltext = "30527-09a.html"
        end
      when BLACKSMITH_PINTER
        if has_quest_items?(player, PASS_1ST_CERTIFICATE, KLUTOS_LETTER)
          htmltext = "30298-01.html"
        elsif has_quest_items?(player, PASS_1ST_CERTIFICATE, FOOTPRINT_OF_THIEF) && !has_quest_items?(player, STOLEN_SECRET_BOX)
          htmltext = "30298-04.html"
        elsif has_quest_items?(player, PASS_1ST_CERTIFICATE, FOOTPRINT_OF_THIEF, STOLEN_SECRET_BOX)
          htmltext = "30298-05.html"
        elsif has_quest_items?(player, PASS_1ST_CERTIFICATE, PASS_2ND_CERTIFICATE, SECRET_BOX)
          htmltext = "30298-07.html"
        end
      when BLACKSMITH_KLUTO
        if has_quest_items?(player, PASS_1ST_CERTIFICATE) && !has_at_least_one_quest_item?(player, FOOTPRINT_OF_THIEF, KLUTOS_LETTER, PASS_2ND_CERTIFICATE, SECRET_BOX)
          htmltext = "30317-01.html"
        elsif has_quest_items?(player, PASS_1ST_CERTIFICATE) && has_at_least_one_quest_item?(player, KLUTOS_LETTER, FOOTPRINT_OF_THIEF)
