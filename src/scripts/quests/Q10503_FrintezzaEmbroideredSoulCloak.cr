class Scripts::Q10503_FrintezzaEmbroideredSoulCloak < Quest
  # NPC
  private OLF_ADAMS = 32612
  # Monster
  # private FRINTEZZA = 29045
  private SCARLET_VAN_HALISHA = 29047
  # Items
  private FRINTEZZAS_SOUL_FRAGMENT = 21724
  private SOUL_CLOAK_OF_FRINTEZZA = 21721
  # Misc
  private MIN_LEVEL = 80
  private FRAGMENT_COUNT = 20

  def initialize
    super(10503, self.class.simple_name, "Frintezza Embroidered Soul Cloak")

    add_start_npc(OLF_ADAMS)
    add_talk_id(OLF_ADAMS)
    add_kill_id(SCARLET_VAN_HALISHA)
    register_quest_items(FRINTEZZAS_SOUL_FRAGMENT)
  end
  ...
  def on_talk(npc, pc)
    ...
    when State::STARTED
      ...
      when 1
        html = "32612-05.html"
      when 2
        if get_quest_items_count(pc, FRINTEZZAS_SOUL_FRAGMENT) >= FRAGMENT_COUNT
          give_items(pc, SOUL_CLOAK_OF_FRINTEZZA, 1)
          play_sound(pc, Sound::ITEMSOUND_QUEST_ITEMGET)
