class Scripts::Q10502_FreyaEmbroideredSoulCloak < Quest
  # NPC
  private OLF_ADAMS = 32612
  # Monster
  private FREYA = 29179
  # Items
  private FREYAS_SOUL_FRAGMENT = 21723
  private SOUL_CLOAK_OF_FREYA = 21720
  # Misc
  private MIN_LEVEL = 82
  private FRAGMENT_COUNT = 20

  def initialize
    super(10502, self.class.simple_name, "Freya Embroidered Soul Cloak")

    add_start_npc(OLF_ADAMS)
    add_talk_id(OLF_ADAMS)
    add_kill_id(FREYA)
    register_quest_items(FREYAS_SOUL_FRAGMENT)
  end
  ...
  def on_talk(npc, pc)
    ...
    when State::STARTED
      ...
      when 1
        html = "32612-05.html"
      when 2
        if get_quest_items_count(pc, FREYAS_SOUL_FRAGMENT) >= FRAGMENT_COUNT
          give_items(pc, SOUL_CLOAK_OF_FREYA, 1)
          play_sound(pc, Sound::ITEMSOUND_QUEST_ITEMGET)
