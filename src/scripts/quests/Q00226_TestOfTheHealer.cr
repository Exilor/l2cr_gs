class Scripts::Q00226_TestOfTheHealer < Quest
  # NPCs
  ...
  def initialize
    ...
    add_talk_id(
      PRIEST_BANDELLOS, MASTER_SORIUS, ALLANA, PERRIN, FATHER_GUPU, ORPHAN_GIRL,
      WINDY_SHAORING, MYSTERIOUS_DARK_ELF, PIPER_LONGBOW, SLEIN_SHINING_BLADE,
      CAIN_FLYING_KNIFE, SAINT_KRISTINA, DAURIN_HAMMERCRUSH
    )
    add_kill_id(
      LERO_LIZARDMAN_AGENT, LERO_LIZARDMAN_LEADER, LERO_LIZARDMAN_ASSASSIN,
      LERO_LIZARDMAN_SNIPER, LERO_LIZARDMAN_WIZARD, LERO_LIZARDMAN_LORD, TATOMA
    )
    register_quest_items(
      REPORT_OF_PERRIN, CRISTINAS_LETTER, PICTURE_OF_WINDY, GOLDEN_STATUE,
      WINDYS_PEBBLES, ORDER_OF_SORIUS, SECRET_LETTER1, SECRET_LETTER2,
      SECRET_LETTER3, SECRET_LETTER4
    )
  end
  ...
  def on_adv_event(event, npc, pc)
    ...
    when "ACCEPT"
      if qs.created?
        ...
        give_items(pc, REPORT_OF_PERRIN, 1)
        if pc.variables.get_i32("2ND_CLASS_DIAMOND_REWARD", 0) == 0
          if pc.class_id.cleric?
            ...
            give_items(pc, DIMENSIONAL_DIAMOND, 45)
          else
    ...
    when "30428-02.html"
      if qs.memo_state?(1) && has_quest_items?(pc, REPORT_OF_PERRIN)
        npc = npc.not_nil!
        qs.set_cond(2, true)
    ...
    when "30660-03.html"
      if has_quest_items?(pc, PICTURE_OF_WINDY)
        npc = npc.not_nil!
        take_items(pc, PICTURE_OF_WINDY, 1)
        give_items(pc, WINDYS_PEBBLES, 1)
        qs.set_cond(8, true)
        npc.delete_me
        html = event
      end
    when "30665-02.html"
      if get_quest_items_count(pc, SECRET_LETTER1) + get_quest_items_count(pc, SECRET_LETTER2) + get_quest_items_count(pc, SECRET_LETTER3) + get_quest_items_count(pc, SECRET_LETTER4) == 4
        ...
        take_items(pc, SECRET_LETTER3, 1)
        take_items(pc, SECRET_LETTER4, 1)
        qs.memo_state = 9
        qs.set_cond(22, true)
        html = event
      end
  ...
  def on_talk(npc, pc)
    ...
    if qs.created?
      if npc.id == PRIEST_BANDELLOS
        if pc.in_category?(CategoryType::WHITE_MAGIC_GROUP)
      ...
      when MASTER_SORIUS
        if memo_state == 5
          give_items(pc, ORDER_OF_SORIUS, 1)
          qs.memo_state = 6
          qs.set_cond(10, true)
          html = "30327-01.html"
          ...
          if has_quest_items?(pc, CRISTINAS_LETTER)
            take_items(pc, CRISTINAS_LETTER, 1)
            qs.memo_state = 10
            qs.set_cond(23, true)
            html = "30327-03.html"
          end
      ...
      when PERRIN
        if memo_state == 1
          if has_quest_items?(pc, REPORT_OF_PERRIN)
            html = "30428-01.html"
          end
        elsif memo_state == 2
          take_items(pc, REPORT_OF_PERRIN, 1)
          qs.memo_state = 3
          qs.set_cond(4, true)
          html = "30428-03.html"
        elsif memo_state == 3
      ...
      when FATHER_GUPU
        if memo_state == 4
          if !has_at_least_one_quest_item?(pc, PICTURE_OF_WINDY, WINDYS_PEBBLES, GOLDEN_STATUE)
            qs.set_cond(6, true)
            html = "30658-01.html"
          elsif has_quest_items?(pc, PICTURE_OF_WINDY)
            html = "30658-04.html"
          elsif has_quest_items?(pc, WINDYS_PEBBLES)
            give_items(pc, GOLDEN_STATUE, 1)
            take_items(pc, WINDYS_PEBBLES, 1)
            qs.memo_state = 5
            html = "30658-06.html"
          end
        elsif memo_state == 5
      ...
      when PIPER_LONGBOW
        if memo_state == 8
          if has_quest_items?(pc, SECRET_LETTER1) && !has_quest_items?(pc, SECRET_LETTER2)
            html = "30662-01.html"
          elsif has_quest_items?(pc, SECRET_LETTER2) && !has_quest_items?(pc, SECRET_LETTER3, SECRET_LETTER4)
            html = "30662-02.html"
          elsif has_quest_items?(pc, SECRET_LETTER2, SECRET_LETTER3, SECRET_LETTER4)
            qs.set_cond(21, true)
            html = "30662-03.html"
          end
        end
      ...
      when CAIN_FLYING_KNIFE
        if memo_state == 8
          if has_quest_items?(pc, SECRET_LETTER1) && !has_quest_items?(pc, SECRET_LETTER4)
            html = "30664-01.html"
          elsif has_quest_items?(pc, SECRET_LETTER2) && !has_quest_items?(pc, SECRET_LETTER3, SECRET_LETTER4)
            html = "30664-02.html"
          elsif has_quest_items?(pc, SECRET_LETTER2, SECRET_LETTER3, SECRET_LETTER4)
      ...
      when DAURIN_HAMMERCRUSH
        if memo_state == 6
          if has_quest_items?(pc, ORDER_OF_SORIUS)
            html = "30674-01.html"
