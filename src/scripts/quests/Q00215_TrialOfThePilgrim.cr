class Quests::Q00215_TrialOfThePilgrim < Quest
  # NPCs
  ...
  def initialize
    ...
    add_talk_id(HERMIT_SANTIAGO, PRIEST_PETRON, PRIEST_PRIMOS, ANDELLIA, GAURI_TWINKLEROCK, SEER_TANAPI, ELDER_CASIAN, ANCESTOR_MARTANKUS, PRIEST_OF_THE_EARTH_GERALD, WANDERER_DORF, URUHA)
    add_kill_id(LAVA_SALAMANDER, NAHIR, BLACK_WILLOW)
    register_quest_items(BOOK_OF_SAGE, VOUCHER_OF_TRIAL, SPIRIT_OF_FLAME, ESSENCE_OF_FLAME, BOOK_OF_GERALD, GREY_BADGE, PICTURE_OF_NAHIR, HAIR_OF_NAHIR, STATUE_OF_EINHASAD, BOOK_OF_DARKNESS, DEBRIS_OF_WILLOW, TAG_OF_RUMOR)
  end
  ...
  def on_adv_event(event, npc, player)
    ...
    when "30648-05.html", "30648-06.html", "30648-07.html", "30648-08.html"
      htmltext = event
    when "30362-05.html"
      if qs.memo_state?(15) && has_quest_items?(player, BOOK_OF_DARKNESS)
        take_items(player, BOOK_OF_DARKNESS, 1)
        qs.memo_state = 16
        qs.set_cond(16, true)
        htmltext = event
      end
    ...
    when "30649-04.html"
      if qs.memo_state?(4) && has_quest_items?(player, ESSENCE_OF_FLAME)
        give_items(player, SPIRIT_OF_FLAME, 1)
        take_items(player, ESSENCE_OF_FLAME, 1)
        qs.memo_state = 5
        qs.set_cond(5, true)
        htmltext = event
      end
    when "30650-02.html"
      if qs.memo_state?(6) && has_quest_items?(player, TAG_OF_RUMOR)
        if get_quest_items_count(player, ADENA) >= 100000
          give_items(player, BOOK_OF_GERALD, 1)
    ...
    when "30652-02.html"
      if qs.memo_state?(14) && has_quest_items?(player, DEBRIS_OF_WILLOW)
        give_items(player, BOOK_OF_DARKNESS, 1)
        take_items(player, DEBRIS_OF_WILLOW, 1)
        qs.memo_state = 15
        qs.set_cond(15, true)
        htmltext = event
      end
  ...
  def on_talk(npc, player)
    ...
    if qs.created?
      if npc.id == HERMIT_SANTIAGO
        if !player.in_category?(CategoryType::HEAL_GROUP)
      ...
      when HERMIT_SANTIAGO
        if memo_state >= 1
          if !has_quest_items?(player, BOOK_OF_SAGE)
            htmltext = "30648-09.html"
          else
            give_adena(player, 229298, true)
            give_items(player, MARK_OF_PILGRIM, 1)
            add_exp_and_sp(player, 1258250, 81606)
      ...
      when PRIEST_PETRON
        if memo_state == 9
          give_items(player, PICTURE_OF_NAHIR, 1)
          qs.memo_state = 10
          qs.set_cond(10, true)
          ...
          take_items(player, PICTURE_OF_NAHIR, 1)
          take_items(player, HAIR_OF_NAHIR, 1)
          give_items(player, STATUE_OF_EINHASAD, 1)
          qs.memo_state = 12
          qs.set_cond(12, true)
          htmltext = "30036-03.html"
        elsif memo_state == 12
          if has_quest_items?(player, STATUE_OF_EINHASAD)
      ...
      when ANDELLIA
        if memo_state == 12
          ...
          if has_quest_items?(player, BOOK_OF_DARKNESS)
      ...
      when GAURI_TWINKLEROCK
        if memo_state == 5
          if has_quest_items?(player, SPIRIT_OF_FLAME)
            take_items(player, SPIRIT_OF_FLAME, 1)
            give_items(player, TAG_OF_RUMOR, 1)
            qs.memo_state = 6
            qs.set_cond(7, true)
      ...
      when SEER_TANAPI
        if memo_state == 1
          if has_quest_items?(player, VOUCHER_OF_TRIAL)
            take_items(player, VOUCHER_OF_TRIAL, 1)
            qs.memo_state = 2
            qs.set_cond(2, true)
      ...
      when ELDER_CASIAN
        if memo_state == 16
          qs.memo_state = 17
          if !has_quest_items?(player, BOOK_OF_SAGE)
            give_items(player, BOOK_OF_SAGE, 1)
          end
          take_items(player, GREY_BADGE, 1)
          take_items(player, SPIRIT_OF_FLAME, 1)
          take_items(player, STATUE_OF_EINHASAD, 1)
          if has_quest_items?(player, BOOK_OF_DARKNESS)
            add_exp_and_sp(player, 5000, 500)
            take_items(player, BOOK_OF_DARKNESS, 1)
          end
      ...
      when ANCESTOR_MARTANKUS
        if memo_state == 2
          ...
          if has_quest_items?(player, ESSENCE_OF_FLAME)
            htmltext = "30649-03.html"
          end
        end
      when PRIEST_OF_THE_EARTH_GERALD
        if memo_state == 6
          if has_quest_items?(player, TAG_OF_RUMOR)
            htmltext = "30650-01.html"
          end
        elsif has_quest_items?(player, GREY_BADGE, BOOK_OF_GERALD)
          give_adena(player, 100000, true)
          take_items(player, BOOK_OF_GERALD, 1)
          htmltext = "30650-04.html"
        end
      when WANDERER_DORF
        if memo_state == 6
          if has_quest_items?(player, TAG_OF_RUMOR)
            give_items(player, GREY_BADGE, 1)
            take_items(player, TAG_OF_RUMOR, 1)
            qs.memo_state = 8
      ...
      when URUHA
        if memo_state == 14
          if has_quest_items?(player, DEBRIS_OF_WILLOW)
