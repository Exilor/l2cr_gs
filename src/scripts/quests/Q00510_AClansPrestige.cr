class Quests::Q00510_AClansPrestige < Quest
  # NPC
  ...
  def on_kill(npc, player, is_summon)
    ...
    if player.clan_leader?
      ...
      if pleader && player.inside_radius?(pleader, 1500, true, false)
  ...
  def on_talk(npc, player)
    ...
    case st.state
    when State::CREATED
      if clan.nil? || (!player.clan_leader? || clan.level < 5)
        html = "31331-0.htm"
      else
        html = "31331-1.htm"
      end
    when State::STARTED
      if clan.nil? || !player.clan_leader?
        st.exit_quest(true)
        return "31331-8.html"
      end
