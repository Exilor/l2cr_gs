class Quests::Q00128_PailakaSongOfIceAndFire < Quest
  # NPCs
  ...
  def on_adv_event(event, npc, player)
    ...
    when "32507-04.htm"
      if st.cond?(3)
        ...
        take_items(player, WATER_ESSENCE, -1)
        take_items(player, BOOK2, -1)
        give_items(player, BOOK3, 1)
    ...
    when "32507-08.htm"
      if st.cond?(6)
        ...
        take_items(player, BOOK5, -1)
        take_items(player, FIRE_ESSENCE, -1)
        give_items(player, ENH_SWORD2, 1)
        give_items(player, BOOK6, 1)
        htmltext = event
      end
    when "32510-02.htm"
      npc = npc.not_nil!
      ...
      if inst.includes?(player.l2id)
        ...
        add_exp_and_sp(player, 810000, 50000)
        REWARDS.each do |id|
          give_items(player, id, 1)
        end
      end
      htmltext = event
    end
    ...
    if st && st.started?
      ...
      when HILLAS
        if st.cond?(2)
          ...
          take_items(player, BOOK1, -1)
          give_items(player, BOOK2, 1)
          give_items(player, WATER_ESSENCE, 1)
        end
        add_spawn(PAPION, -53903, 181484, -4555, 30456, false, 0, false, npc.instance_id)
      when PAPION
        if st.cond?(4)
          st.set_cond(5)
          take_items(player, BOOK3, -1)
          give_items(player, BOOK4, 1)
          play_sound(player, Sound::ITEMSOUND_QUEST_ITEMGET)
        end
      ...
      when KINSUS
        if st.cond?(5)
          st.set_cond(6)
          play_sound(player, Sound::ITEMSOUND_QUEST_ITEMGET)
          take_items(player, BOOK4, -1)
          give_items(player, BOOK5, 1)
          give_items(player, FIRE_ESSENCE, 1)
        end
      ...
      when GARGOS
        if st.cond?(7)
          st.set_cond(8)
          play_sound(player, Sound::ITEMSOUND_QUEST_ITEMGET)
          take_items(player, BOOK6, -1)
          give_items(player, BOOK7, 1)
        end
