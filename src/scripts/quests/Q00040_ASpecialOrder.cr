class Quests::Q00040_ASpecialOrder < Quest
  # NPCs
  ...
  def on_talk(npc, player)
    ...
    when HELVETIA
      case st.state
      when State::CREATED
        htmltext = player.level >= MIN_LVL ? "30081-01.htm" : "30081-02.htm"
      when State::STARTED
        case st.cond
        when 2, 3
    ...
    when OFULLE
      ...
      when 3
        if st.get_quest_items_count(ORANGE_SWIFT_FISH) >= 10 && st.get_quest_items_count(ORANGE_UGLY_FISH) >= 10 && st.get_quest_items_count(ORANGE_WIDE_FISH) >= 10
          ...
          take_items(player, 10, {ORANGE_SWIFT_FISH, ORANGE_UGLY_FISH, ORANGE_WIDE_FISH})
    ...
    when GESTO
      ...
      when 6
        if st.get_quest_items_count(GOLDEN_COBOL) >= 40 && st.get_quest_items_count(BUR_COBOL) >= 40 && st.get_quest_items_count(GREAT_COBOL) >= 40
          ...
          take_items(player, 40, {GOLDEN_COBOL, BUR_COBOL, GREAT_COBOL})
