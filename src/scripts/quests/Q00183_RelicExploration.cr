class Scripts::Q00183_RelicExploration < Quest
  # NPCs
  ...
  def on_adv_event(event, npc, pc)
    ...
    when "Contract"
      qs184 = pc.get_quest_state(Q00184_ArtOfPersuasion.simple_name)
      qs185 = pc.get_quest_state(Q00185_NikolasCooperation.simple_name)
      quest = QuestManager.get_quest(Q00184_ArtOfPersuasion.simple_name)
      if quest && qs184.nil? && qs185.nil?
        if pc.level >= MIN_LEVEL
          quest.notify_event("30621-03.htm", npc, pc)
        else
  ...
  def on_talk(npc, pc)
    ...
    if qs.created?
      if npc.id == HEAD_BLACKSMITH_KUSTO
        html = pc.level >= MIN_LEVEL ? "30512-01.htm" : "30512-03.html"
      end
    elsif qs.started?
      case npc.id
