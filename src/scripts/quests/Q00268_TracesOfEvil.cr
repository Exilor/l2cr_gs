class Quests::Q00268_TracesOfEvil < Quest
  # NPC
  private KUNAI = 30559
  # Item
  ...
  def on_talk(npc, pc)
    ...
    if st
      case st.state
      when State::CREATED
        htmltext = pc.level >= MIN_LVL ? "30559-02.htm" : "30559-01.htm"
      when State::STARTED
        case st.cond
        when 1
          htmltext = !st.has_quest_items?(CONTAMINATED_KASHA_SPIDER_VENOM) ? "30559-04.html" : "30559-05.html"
        when 2
          if st.get_quest_items_count(CONTAMINATED_KASHA_SPIDER_VENOM) >= 30
