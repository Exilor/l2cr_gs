class Scripts::JiniaGuildHideout4 < AbstractInstance
  private class JGH4World < InstanceWorld
  end
  ...
  def on_talk(npc, talker)
    qs = talker.get_quest_state(Q10287_StoryOfThoseLeft.simple_name)
    if qs && qs.memo_state?(1)
      enter_instance(talker, JGH4World.new, "JiniaGuildHideout4.xml", TEMPLATE_ID)
      qs.set_cond(2, true)
    end
