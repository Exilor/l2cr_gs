class L2OlympiadStadiumZone < L2ZoneRespawn
  ...
  #   @spectator_locations = nil # Location[]
  ...
  def on_enter(char)
    warn "TODO" if char.player?
  #   if settings.olympiad_task &.battle_started?
  ...
  def parse_loc(x, y, z, type)
    if type && type == "spectatorSpawn"
      (@spectator_locations ||= [] of Location) << Location.new(x, y, z)
    else
      super
    end
  end
  ...
  # class Settings
  #   attr_accessor :task
  #   alias olympiad_task task

  #   def initialize
  #     @task = nil # OlympiadGameTask
  #   end
