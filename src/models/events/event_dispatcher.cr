...
module EventDispatcher
  ...
  def notify(event : BaseEvent, callback_class : (T.class)?) forall T
    notify(event, nil, callback_class)
  end

  def notify(event : BaseEvent, container : ListenersContainer)
    notify(event, container, nil)
  end

  def notify(event : BaseEvent, container : ListenersContainer?, callback_class : (T.class)?) forall T
    ...
    error "Couldn't notify event #{event} with callback #{callback_class} and container #{container}."
    error e
    nil
  end
  ...
  private def notify_impl(event : BaseEvent, container : ListenersContainer?, callback_class : (T.class)?) forall T
    callback = nil
  ...
  # private def to_multiple_containers(event : BaseEvent, containers : Enumerable(ListenersContainer)?, callback_class) # : T? forall T
  #   callback = nil
  ...
  private def to_multiple_containers(event : BaseEvent, containers : Enumerable(ListenersContainer)?, callback_class : (AbstractEventReturn.class)?) : AbstractEventReturn? # : T? forall T
    callback = nil
    containers.try &.each do |container|
      if !callback || !callback.abort
        callback = to_listeners(container.get_listeners(event.type), event, callback_class, callback)
...
#   def notify(event : BaseEvent, callback_class : (AbstractEventReturn.class)?)  : AbstractEventReturn?
#     notify(event, nil, callback_class)
#   end
...
#   def notify(event : BaseEvent, container : ListenersContainer?, callback_class : (AbstractEventReturn.class)?) : AbstractEventReturn?
#     if Containers::GLOBAL.has_listener?(event.type) || (container.try &.has_listener?(event.type))
#       notify_impl(event, container, callback_class)
#     end
#   rescue e
...
#   private def notify_impl(event : BaseEvent, container : ListenersContainer?, callback_class : (AbstractEventReturn.class)?) : AbstractEventReturn?
#     callback : AbstractEventReturn? = nil
...
#   private def to_listeners(listeners : Enumerable(AbstractEventListener), event : BaseEvent, return_class : (AbstractEventReturn.class)?, callback : AbstractEventReturn?) : AbstractEventReturn?
#     listeners.each do |listener|
#       rb = listener.execute_event(event, return_class)
