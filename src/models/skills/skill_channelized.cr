struct SkillChannelized
  private EMPTY_HASH = {} of Int32 => L2Character
  ...
  def remove_channelizer(skill_id : Int32, channelizer : L2Character)
    get_channelizers(skill_id).delete(channelizer.l2id)
  end

  def get_channelizers_size(skill_id : Int32) : Int32
    get_channelizers(skill_id).size
  end

  def abort_channelization
    # @channelizers.each_value &.each_value &.abort_cast
    @channelizers.clear
  end

  def get_channelizers(skill_id : Int32) : Hash(Int32, L2Character)
    @channelizers.fetch(skill_id, EMPTY_HASH)
  end
