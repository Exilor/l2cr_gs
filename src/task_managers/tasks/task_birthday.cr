class TaskBirthday < Task
  ...
  def init
    super
    TaskManager.add_unique_task(name, TaskType::TYPE_GLOBAL_TASK, "1", "06:30:00", "")
  end
  ...
  def on_time_elapsed(task)
    gift_count = give_birthday_gifts(task.last_activation)
    info "#{gift_count} gifts sent."
  end

  private def give_birthday_gifts(last_activation) : Int32
    gift_count = 0
    ...
    begin
      GameDB.each(SELECT_PENDING_BIRTHDAY_GIFTS, Time.s_to_ms(last_activation)) do |rs|
        ...
        msg.create_attachments.add_item("Birthday", Config.alt_birthday_gift, 1, nil, nil)
