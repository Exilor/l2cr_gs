...
module ThreadPoolManager
  ...
  def schedule_effect(task, delay) : TaskExecutor::Scheduler::DelayedTask
    schedule_delayed(task, delay, effects_scheduled_thread_pool)
  end

  def schedule_effect_at_fixed_rate(task, delay, interval) : TaskExecutor::Scheduler::PeriodicTask
    schedule_periodic(task, delay, interval, effects_scheduled_thread_pool)
  end

  def schedule_general(task, delay) : TaskExecutor::Scheduler::DelayedTask
    schedule_delayed(task, delay, general_scheduled_thread_pool)
  end

  def schedule_general_at_fixed_rate(task, delay, interval) : TaskExecutor::Scheduler::PeriodicTask
    schedule_periodic(task, delay, interval, general_scheduled_thread_pool)
  end

  def schedule_event(task, delay) : TaskExecutor::Scheduler::DelayedTask
    schedule_delayed(task, delay, event_scheduled_thread_pool)
  end

  def schedule_event_at_fixed_rate(task, delay, interval) : TaskExecutor::Scheduler::PeriodicTask
    schedule_periodic(task, delay, interval, event_scheduled_thread_pool)
  end

  def schedule_ai(task, delay) : TaskExecutor::Scheduler::DelayedTask
    schedule_delayed(task, delay, ai_scheduled_thread_pool)
  end

  def schedule_ai_at_fixed_rate(task, delay, interval) : TaskExecutor::Scheduler::PeriodicTask
