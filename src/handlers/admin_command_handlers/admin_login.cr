module AdminCommandHandler::AdminLogin
  ...
  def use_admin_command(command, pc)
    if command == "admin_server_gm_only"
      ...
      if st.size > 1
        ...
        number = st.shift
        begin
          LoginServerThread.instance.max_players = number.to_i
      ...
      if tokens > 1
        ...
        if Config.server_list_type != new_type
          ...
          LoginServerThread.instance.send_server_type
      ...
      if st.size > 1
        ...
        begin
          ...
          if Config.server_list_age != age
            ...
            LoginServerThread.instance.send_server_status(ServerStatus::SERVER_AGE, age)
  ...
  private def show_main_page(pc)
    ...
    html["%server_name%"] = LoginServerThread.instance.server_name
    html["%status%"] = LoginServerThread.instance.status_string
    html["%clock%"] = get_server_type_name(Config.server_list_type)
    html["%brackets%"] = Config.server_list_bracket
    html["%max_players%"] = LoginServerThread.instance.max_players
    pc.send_packet(html)
  end
  ...
  private def gm_only
    LoginServerThread.instance.server_status = ServerStatus::STATUS_GM_ONLY
