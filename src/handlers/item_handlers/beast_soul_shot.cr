module ItemHandler::BeastSoulShot
  extend self
  extend ItemHandler
  ...
  def use_item(playable, item, force) : Bool
    ...
    item_id = item.id
    shot_consumption = summon.soulshots_per_hit
    ...
    if summon.charged_shot?(ShotType::SOULSHOTS)
      return false
    end
    ...
    summon.set_charged_shot(ShotType::SOULSHOTS, true)
