module ItemHandler::BlessedSpiritShot
  extend self
  extend ItemHandler
  ...
  def use_item(playable : L2Playable, item : L2ItemInstance, force_use : Bool) : Bool
    ...
    if pc.charged_shot?(ShotType::BLESSED_SPIRITSHOTS)
      return false
    end

    unless item.etc_item? && item.template.default_action.spiritshot? && weapon_inst.template.item_grade_s_plus == item.template.item_grade_s_plus
      unless pc.active_shots.includes?(item_id)
    ...
    pc.set_charged_shot(ShotType::BLESSED_SPIRITSHOTS, true)
    sm = SystemMessage.use_s1_
    sm.add_item_name(item_id)
    pc.send_packet(sm)

    pc.send_packet(SystemMessageId::ENABLED_SPIRITSHOT)
