class EffectHandler::ResurrectionSpecial < AbstractEffect
  ...
  def effect_type
    L2EffectType::RESURRECTION_SPECIAL
  end
  ...
  def on_exit(info)
    ...
    return if !effected.player? && !effected.pet?

    caster = effector.acting_player

    if effected.player?
      effected.acting_player.revive_request(caster, skill, false, @res_power, @res_recovery)
      return
    end

    if effected.is_a?(L2PetInstance)
