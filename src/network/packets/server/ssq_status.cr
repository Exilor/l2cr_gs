class Packets::Outgoing::SSQStatus < GameServerPacket
  ...
  private def write_impl
    ...
    case @page
    when 1
      d SevenSigns.current_cycle

      current_period = SevenSigns.current_period
      ...
      q SevenSigns.get_player_stone_contrib(@l2id)
      q SevenSigns.get_player_adena_collect(@l2id)

      dawn_stone_score = SevenSigns.get_current_stone_score(SevenSigns::CABAL_DAWN).to_f
      dawn_festival_score = SevenSigns.get_current_festival_score(SevenSigns::CABAL_DAWN)
      dusk_stone_score = SevenSigns.get_current_stone_score(SevenSigns::CABAL_DUSK).to_f
      dusk_festival_score = SevenSigns.get_current_festival_score(SevenSigns::CABAL_DUSK)
      ...
      dusk_total_score = SevenSigns.get_current_score(SevenSigns::CABAL_DUSK)
      dawn_total_score = SevenSigns.get_current_score(SevenSigns::CABAL_DAWN)
    ...
    when 2
      ...
      5.times do |i|
        ...
        high_score_data = SevenSignsFestival.get_highest_score_data(SevenSigns::CABAL_DUSK, i)
        ...
        high_score_data = SevenSignsFestival.get_highest_score_data(SevenSigns::CABAL_DAWN, i)
        party_members = high_score_data.get_string("members").split(',')
    ...
    when 3
      ...
      1.upto(3) do |i|
        dawn_proportion = SevenSigns.get_seal_proportion(i, SevenSigns::CABAL_DAWN)
        dusk_proportion = SevenSigns.get_seal_proportion(i, SevenSigns::CABAL_DUSK)

        c i
        c SevenSigns.get_seal_owner(i)
