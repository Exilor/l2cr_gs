class Packets::Incoming::Attack < GameClientPacket
  @l2id = 0
  @origin_x = 0
  @origin_y = 0
  @origin_z = 0
  ...
  private def run_impl
    ...
    if pc.playable? && pc.in_boat?
      pc.send_packet(SystemMessageId::NOT_ALLOWED_ON_BOAT)
      action_failed
      return
    end
    ...
    if !target.targetable? && !pc.override_target_all?
      action_failed
      return
    elsif target.instance_id != pc.instance_id && pc.instance_id != -1
      action_failed
      return
    elsif !target.visible_for?(pc)
      action_failed
      return
    end
    ...
    if pc.target != target
      ...
      if target.l2id != pc.l2id && pc.private_store_type.none? && !pc.active_requester
